import qbs 1.0

//Project {
//    minimumQbsVersion: "1.6.0"
Product {

    consoleApplication: true

    cpp.debugInformation: true
    cpp.executableSuffix: ".elf"
    cpp.positionIndependentCode: false

    property string mfloatAbi: "-mfloat-abi=hard"
    property string mcpu: "-mcpu=cortex-m4"
    property string mfpu: "-mfpu=fpv4-sp-d16"

    type: ["application", "flash"]

    Depends { name:"cpp" }
    Group {
        name: "app"
        prefix: path
        files: [
            "/src/**/*.c",
            "/src/**/*.cpp",
            "/src/**/*.h",
        ]
        excludeFiles: [
        ]
    }
    Group {
        name: "mbed-os/drivers"
        files: [
            "mbed-os/drivers/AnalogIn.cpp",
            "mbed-os/drivers/AnalogIn.h",
            "mbed-os/drivers/AnalogOut.h",
            "mbed-os/drivers/BusIn.cpp",
            "mbed-os/drivers/BusIn.h",
            "mbed-os/drivers/BusInOut.cpp",
            "mbed-os/drivers/BusInOut.h",
            "mbed-os/drivers/BusOut.cpp",
            "mbed-os/drivers/BusOut.h",
            "mbed-os/drivers/CAN.cpp",
            "mbed-os/drivers/CAN.h",
            "mbed-os/drivers/DigitalIn.h",
            "mbed-os/drivers/DigitalInOut.h",
            "mbed-os/drivers/DigitalOut.h",
            "mbed-os/drivers/Ethernet.cpp",
            "mbed-os/drivers/Ethernet.h",
            "mbed-os/drivers/FlashIAP.cpp",
            "mbed-os/drivers/FlashIAP.h",
            "mbed-os/drivers/I2C.cpp",
            "mbed-os/drivers/I2C.h",
            "mbed-os/drivers/I2CSlave.cpp",
            "mbed-os/drivers/I2CSlave.h",
            "mbed-os/drivers/InterruptIn.cpp",
            "mbed-os/drivers/InterruptIn.h",
            "mbed-os/drivers/InterruptManager.cpp",
            "mbed-os/drivers/InterruptManager.h",
            "mbed-os/drivers/LowPowerTicker.h",
            "mbed-os/drivers/LowPowerTimeout.h",
            "mbed-os/drivers/LowPowerTimer.h",
            "mbed-os/drivers/PortIn.h",
            "mbed-os/drivers/PortInOut.h",
            "mbed-os/drivers/PortOut.h",
            "mbed-os/drivers/PwmOut.h",
            "mbed-os/drivers/RawSerial.cpp",
            "mbed-os/drivers/RawSerial.h",
            "mbed-os/drivers/SPI.cpp",
            "mbed-os/drivers/SPI.h",
            "mbed-os/drivers/SPISlave.cpp",
            "mbed-os/drivers/SPISlave.h",
            "mbed-os/drivers/Serial.cpp",
            "mbed-os/drivers/Serial.h",
            "mbed-os/drivers/SerialBase.cpp",
            "mbed-os/drivers/SerialBase.h",
            "mbed-os/drivers/Ticker.cpp",
            "mbed-os/drivers/Ticker.h",
            "mbed-os/drivers/Timeout.cpp",
            "mbed-os/drivers/Timeout.h",
            "mbed-os/drivers/Timer.cpp",
            "mbed-os/drivers/Timer.h",
            "mbed-os/drivers/TimerEvent.cpp",
            "mbed-os/drivers/TimerEvent.h",
            "mbed-os/drivers/UARTSerial.cpp",
            "mbed-os/drivers/UARTSerial.h",
        ]

    }

    Group {
        name: "mbed-os"
        files: [
            "mbed-os/cmsis/TARGET_CORTEX_M/arm_math.h",
            "mbed-os/cmsis/TARGET_CORTEX_M/cmsis_armcc.h",
            "mbed-os/cmsis/TARGET_CORTEX_M/cmsis_armclang.h",
            "mbed-os/cmsis/TARGET_CORTEX_M/cmsis_compiler.h",
            "mbed-os/cmsis/TARGET_CORTEX_M/cmsis_gcc.h",
            "mbed-os/cmsis/TARGET_CORTEX_M/cmsis_iccarm.h",
            "mbed-os/cmsis/TARGET_CORTEX_M/cmsis_version.h",
            "mbed-os/cmsis/TARGET_CORTEX_M/core_armv8mbl.h",
            "mbed-os/cmsis/TARGET_CORTEX_M/core_armv8mml.h",
            "mbed-os/cmsis/TARGET_CORTEX_M/core_cm0.h",
            "mbed-os/cmsis/TARGET_CORTEX_M/core_cm0plus.h",
            "mbed-os/cmsis/TARGET_CORTEX_M/core_cm23.h",
            "mbed-os/cmsis/TARGET_CORTEX_M/core_cm3.h",
            "mbed-os/cmsis/TARGET_CORTEX_M/core_cm33.h",
            "mbed-os/cmsis/TARGET_CORTEX_M/core_cm4.h",
            "mbed-os/cmsis/TARGET_CORTEX_M/core_cm7.h",
            "mbed-os/cmsis/TARGET_CORTEX_M/core_cmSecureAccess.h",
            "mbed-os/cmsis/TARGET_CORTEX_M/core_sc000.h",
            "mbed-os/cmsis/TARGET_CORTEX_M/core_sc300.h",
            "mbed-os/cmsis/TARGET_CORTEX_M/mpu_armv7.h",
            "mbed-os/cmsis/TARGET_CORTEX_M/mpu_armv8.h",
            "mbed-os/cmsis/TARGET_CORTEX_M/tz_context.h",
            "mbed-os/events/Event.h",
            "mbed-os/events/EventQueue.cpp",
            "mbed-os/events/EventQueue.h",
            "mbed-os/events/equeue/equeue.c",
            "mbed-os/events/equeue/equeue.h",
            "mbed-os/events/equeue/equeue_mbed.cpp",
            "mbed-os/events/equeue/equeue_platform.h",
            "mbed-os/events/equeue/equeue_posix.c",
            "mbed-os/events/mbed_events.h",
            "mbed-os/events/mbed_shared_queues.cpp",
            "mbed-os/events/mbed_shared_queues.h",
            "mbed-os/features/filesystem/Dir.cpp",
            "mbed-os/features/filesystem/Dir.h",
            "mbed-os/features/filesystem/File.cpp",
            "mbed-os/features/filesystem/File.h",
            "mbed-os/features/filesystem/FileSystem.cpp",
            "mbed-os/features/filesystem/FileSystem.h",
            "mbed-os/features/filesystem/bd/BlockDevice.h",
            "mbed-os/features/filesystem/bd/ChainingBlockDevice.cpp",
            "mbed-os/features/filesystem/bd/ChainingBlockDevice.h",
            "mbed-os/features/filesystem/bd/HeapBlockDevice.cpp",
            "mbed-os/features/filesystem/bd/HeapBlockDevice.h",
            "mbed-os/features/filesystem/bd/MBRBlockDevice.cpp",
            "mbed-os/features/filesystem/bd/MBRBlockDevice.h",
            "mbed-os/features/filesystem/bd/ProfilingBlockDevice.cpp",
            "mbed-os/features/filesystem/bd/ProfilingBlockDevice.h",
            "mbed-os/features/filesystem/bd/SlicingBlockDevice.cpp",
            "mbed-os/features/filesystem/bd/SlicingBlockDevice.h",
            "mbed-os/features/filesystem/fat/ChaN/ccsbcs.cpp",
            "mbed-os/features/filesystem/fat/ChaN/diskio.h",
            "mbed-os/features/filesystem/fat/ChaN/ff.cpp",
            "mbed-os/features/filesystem/fat/ChaN/ff.h",
            "mbed-os/features/filesystem/fat/ChaN/ffconf.h",
            "mbed-os/features/filesystem/fat/ChaN/integer.h",
            "mbed-os/features/filesystem/fat/FATFileSystem.cpp",
            "mbed-os/features/filesystem/fat/FATFileSystem.h",
            "mbed-os/features/filesystem/mbed_filesystem.h",
            "mbed-os/features/frameworks/greentea-client/greentea-client/greentea_metrics.h",
            "mbed-os/features/frameworks/greentea-client/greentea-client/greentea_serial.h",
            "mbed-os/features/frameworks/greentea-client/greentea-client/test_env.h",
            "mbed-os/features/frameworks/greentea-client/source/greentea_metrics.cpp",
            "mbed-os/features/frameworks/greentea-client/source/greentea_serial.cpp",
            "mbed-os/features/frameworks/greentea-client/source/greentea_test_env.cpp",
            "mbed-os/features/frameworks/unity/source/unity.c",
            "mbed-os/features/frameworks/unity/unity/unity.h",
            "mbed-os/features/frameworks/unity/unity/unity_config.h",
            "mbed-os/features/frameworks/unity/unity/unity_internals.h",
            "mbed-os/features/frameworks/utest/mbed-utest-shim.cpp",
            "mbed-os/features/frameworks/utest/source/unity_handler.cpp",
            "mbed-os/features/frameworks/utest/source/utest_case.cpp",
            "mbed-os/features/frameworks/utest/source/utest_default_handlers.cpp",
            "mbed-os/features/frameworks/utest/source/utest_greentea_handlers.cpp",
            "mbed-os/features/frameworks/utest/source/utest_harness.cpp",
            "mbed-os/features/frameworks/utest/source/utest_shim.cpp",
            "mbed-os/features/frameworks/utest/source/utest_stack_trace.cpp",
            "mbed-os/features/frameworks/utest/source/utest_types.cpp",
            "mbed-os/features/frameworks/utest/utest/unity_handler.h",
            "mbed-os/features/frameworks/utest/utest/utest.h",
            "mbed-os/features/frameworks/utest/utest/utest_case.h",
            "mbed-os/features/frameworks/utest/utest/utest_default_handlers.h",
            "mbed-os/features/frameworks/utest/utest/utest_harness.h",
            "mbed-os/features/frameworks/utest/utest/utest_scheduler.h",
            "mbed-os/features/frameworks/utest/utest/utest_serial.h",
            "mbed-os/features/frameworks/utest/utest/utest_shim.h",
            "mbed-os/features/frameworks/utest/utest/utest_specification.h",
            "mbed-os/features/frameworks/utest/utest/utest_stack_trace.h",
            "mbed-os/features/frameworks/utest/utest/utest_types.h",
            "mbed-os/features/mbedtls/inc/mbedtls/aes.h",
            "mbed-os/features/mbedtls/inc/mbedtls/aesni.h",
            "mbed-os/features/mbedtls/inc/mbedtls/arc4.h",
            "mbed-os/features/mbedtls/inc/mbedtls/asn1.h",
            "mbed-os/features/mbedtls/inc/mbedtls/asn1write.h",
            "mbed-os/features/mbedtls/inc/mbedtls/base64.h",
            "mbed-os/features/mbedtls/inc/mbedtls/bignum.h",
            "mbed-os/features/mbedtls/inc/mbedtls/blowfish.h",
            "mbed-os/features/mbedtls/inc/mbedtls/bn_mul.h",
            "mbed-os/features/mbedtls/inc/mbedtls/camellia.h",
            "mbed-os/features/mbedtls/inc/mbedtls/ccm.h",
            "mbed-os/features/mbedtls/inc/mbedtls/certs.h",
            "mbed-os/features/mbedtls/inc/mbedtls/check_config.h",
            "mbed-os/features/mbedtls/inc/mbedtls/cipher.h",
            "mbed-os/features/mbedtls/inc/mbedtls/cipher_internal.h",
            "mbed-os/features/mbedtls/inc/mbedtls/cmac.h",
            "mbed-os/features/mbedtls/inc/mbedtls/compat-1.3.h",
            "mbed-os/features/mbedtls/inc/mbedtls/config-no-entropy.h",
            "mbed-os/features/mbedtls/inc/mbedtls/config.h",
            "mbed-os/features/mbedtls/inc/mbedtls/ctr_drbg.h",
            "mbed-os/features/mbedtls/inc/mbedtls/debug.h",
            "mbed-os/features/mbedtls/inc/mbedtls/des.h",
            "mbed-os/features/mbedtls/inc/mbedtls/dhm.h",
            "mbed-os/features/mbedtls/inc/mbedtls/ecdh.h",
            "mbed-os/features/mbedtls/inc/mbedtls/ecdsa.h",
            "mbed-os/features/mbedtls/inc/mbedtls/ecjpake.h",
            "mbed-os/features/mbedtls/inc/mbedtls/ecp.h",
            "mbed-os/features/mbedtls/inc/mbedtls/ecp_internal.h",
            "mbed-os/features/mbedtls/inc/mbedtls/entropy.h",
            "mbed-os/features/mbedtls/inc/mbedtls/entropy_poll.h",
            "mbed-os/features/mbedtls/inc/mbedtls/error.h",
            "mbed-os/features/mbedtls/inc/mbedtls/gcm.h",
            "mbed-os/features/mbedtls/inc/mbedtls/havege.h",
            "mbed-os/features/mbedtls/inc/mbedtls/hmac_drbg.h",
            "mbed-os/features/mbedtls/inc/mbedtls/md.h",
            "mbed-os/features/mbedtls/inc/mbedtls/md2.h",
            "mbed-os/features/mbedtls/inc/mbedtls/md4.h",
            "mbed-os/features/mbedtls/inc/mbedtls/md5.h",
            "mbed-os/features/mbedtls/inc/mbedtls/md_internal.h",
            "mbed-os/features/mbedtls/inc/mbedtls/memory_buffer_alloc.h",
            "mbed-os/features/mbedtls/inc/mbedtls/net.h",
            "mbed-os/features/mbedtls/inc/mbedtls/net_sockets.h",
            "mbed-os/features/mbedtls/inc/mbedtls/oid.h",
            "mbed-os/features/mbedtls/inc/mbedtls/padlock.h",
            "mbed-os/features/mbedtls/inc/mbedtls/pem.h",
            "mbed-os/features/mbedtls/inc/mbedtls/pk.h",
            "mbed-os/features/mbedtls/inc/mbedtls/pk_internal.h",
            "mbed-os/features/mbedtls/inc/mbedtls/pkcs11.h",
            "mbed-os/features/mbedtls/inc/mbedtls/pkcs12.h",
            "mbed-os/features/mbedtls/inc/mbedtls/pkcs5.h",
            "mbed-os/features/mbedtls/inc/mbedtls/platform.h",
            "mbed-os/features/mbedtls/inc/mbedtls/platform_time.h",
            "mbed-os/features/mbedtls/inc/mbedtls/ripemd160.h",
            "mbed-os/features/mbedtls/inc/mbedtls/rsa.h",
            "mbed-os/features/mbedtls/inc/mbedtls/sha1.h",
            "mbed-os/features/mbedtls/inc/mbedtls/sha256.h",
            "mbed-os/features/mbedtls/inc/mbedtls/sha512.h",
            "mbed-os/features/mbedtls/inc/mbedtls/ssl.h",
            "mbed-os/features/mbedtls/inc/mbedtls/ssl_cache.h",
            "mbed-os/features/mbedtls/inc/mbedtls/ssl_ciphersuites.h",
            "mbed-os/features/mbedtls/inc/mbedtls/ssl_cookie.h",
            "mbed-os/features/mbedtls/inc/mbedtls/ssl_internal.h",
            "mbed-os/features/mbedtls/inc/mbedtls/ssl_ticket.h",
            "mbed-os/features/mbedtls/inc/mbedtls/threading.h",
            "mbed-os/features/mbedtls/inc/mbedtls/timing.h",
            "mbed-os/features/mbedtls/inc/mbedtls/version.h",
            "mbed-os/features/mbedtls/inc/mbedtls/x509.h",
            "mbed-os/features/mbedtls/inc/mbedtls/x509_crl.h",
            "mbed-os/features/mbedtls/inc/mbedtls/x509_crt.h",
            "mbed-os/features/mbedtls/inc/mbedtls/x509_csr.h",
            "mbed-os/features/mbedtls/inc/mbedtls/xtea.h",
            "mbed-os/features/mbedtls/platform/inc/platform_mbed.h",
            "mbed-os/features/mbedtls/platform/src/mbed_trng.c",
            "mbed-os/features/mbedtls/src/aes.c",
            "mbed-os/features/mbedtls/src/aesni.c",
            "mbed-os/features/mbedtls/src/arc4.c",
            "mbed-os/features/mbedtls/src/asn1parse.c",
            "mbed-os/features/mbedtls/src/asn1write.c",
            "mbed-os/features/mbedtls/src/base64.c",
            "mbed-os/features/mbedtls/src/bignum.c",
            "mbed-os/features/mbedtls/src/blowfish.c",
            "mbed-os/features/mbedtls/src/camellia.c",
            "mbed-os/features/mbedtls/src/ccm.c",
            "mbed-os/features/mbedtls/src/certs.c",
            "mbed-os/features/mbedtls/src/cipher.c",
            "mbed-os/features/mbedtls/src/cipher_wrap.c",
            "mbed-os/features/mbedtls/src/cmac.c",
            "mbed-os/features/mbedtls/src/ctr_drbg.c",
            "mbed-os/features/mbedtls/src/debug.c",
            "mbed-os/features/mbedtls/src/des.c",
            "mbed-os/features/mbedtls/src/dhm.c",
            "mbed-os/features/mbedtls/src/ecdh.c",
            "mbed-os/features/mbedtls/src/ecdsa.c",
            "mbed-os/features/mbedtls/src/ecjpake.c",
            "mbed-os/features/mbedtls/src/ecp.c",
            "mbed-os/features/mbedtls/src/ecp_curves.c",
            "mbed-os/features/mbedtls/src/entropy.c",
            "mbed-os/features/mbedtls/src/entropy_poll.c",
            "mbed-os/features/mbedtls/src/error.c",
            "mbed-os/features/mbedtls/src/gcm.c",
            "mbed-os/features/mbedtls/src/havege.c",
            "mbed-os/features/mbedtls/src/hmac_drbg.c",
            "mbed-os/features/mbedtls/src/md.c",
            "mbed-os/features/mbedtls/src/md2.c",
            "mbed-os/features/mbedtls/src/md4.c",
            "mbed-os/features/mbedtls/src/md5.c",
            "mbed-os/features/mbedtls/src/md_wrap.c",
            "mbed-os/features/mbedtls/src/memory_buffer_alloc.c",
            "mbed-os/features/mbedtls/src/net_sockets.c",
            "mbed-os/features/mbedtls/src/oid.c",
            "mbed-os/features/mbedtls/src/padlock.c",
            "mbed-os/features/mbedtls/src/pem.c",
            "mbed-os/features/mbedtls/src/pk.c",
            "mbed-os/features/mbedtls/src/pk_wrap.c",
            "mbed-os/features/mbedtls/src/pkcs11.c",
            "mbed-os/features/mbedtls/src/pkcs12.c",
            "mbed-os/features/mbedtls/src/pkcs5.c",
            "mbed-os/features/mbedtls/src/pkparse.c",
            "mbed-os/features/mbedtls/src/pkwrite.c",
            "mbed-os/features/mbedtls/src/platform.c",
            "mbed-os/features/mbedtls/src/ripemd160.c",
            "mbed-os/features/mbedtls/src/rsa.c",
            "mbed-os/features/mbedtls/src/sha1.c",
            "mbed-os/features/mbedtls/src/sha256.c",
            "mbed-os/features/mbedtls/src/sha512.c",
            "mbed-os/features/mbedtls/src/ssl_cache.c",
            "mbed-os/features/mbedtls/src/ssl_ciphersuites.c",
            "mbed-os/features/mbedtls/src/ssl_cli.c",
            "mbed-os/features/mbedtls/src/ssl_cookie.c",
            "mbed-os/features/mbedtls/src/ssl_srv.c",
            "mbed-os/features/mbedtls/src/ssl_ticket.c",
            "mbed-os/features/mbedtls/src/ssl_tls.c",
            "mbed-os/features/mbedtls/src/threading.c",
            "mbed-os/features/mbedtls/src/timing.c",
            "mbed-os/features/mbedtls/src/version.c",
            "mbed-os/features/mbedtls/src/version_features.c",
            "mbed-os/features/mbedtls/src/x509.c",
            "mbed-os/features/mbedtls/src/x509_create.c",
            "mbed-os/features/mbedtls/src/x509_crl.c",
            "mbed-os/features/mbedtls/src/x509_crt.c",
            "mbed-os/features/mbedtls/src/x509_csr.c",
            "mbed-os/features/mbedtls/src/x509write_crt.c",
            "mbed-os/features/mbedtls/src/x509write_csr.c",
            "mbed-os/features/mbedtls/src/xtea.c",
            "mbed-os/features/mbedtls/targets/TARGET_STM/aes_alt.c",
            "mbed-os/features/mbedtls/targets/TARGET_STM/aes_alt.h",
            "mbed-os/features/mbedtls/targets/TARGET_STM/md5_alt.c",
            "mbed-os/features/mbedtls/targets/TARGET_STM/md5_alt.h",
            "mbed-os/features/mbedtls/targets/TARGET_STM/sha1_alt.c",
            "mbed-os/features/mbedtls/targets/TARGET_STM/sha1_alt.h",
            "mbed-os/features/mbedtls/targets/TARGET_STM/sha256_alt.c",
            "mbed-os/features/mbedtls/targets/TARGET_STM/sha256_alt.h",
            "mbed-os/features/netsocket/CellularBase.h",
            "mbed-os/features/netsocket/CellularInterface.h",
            "mbed-os/features/netsocket/EthInterface.h",
            "mbed-os/features/netsocket/MeshInterface.h",
            "mbed-os/features/netsocket/NetworkInterface.cpp",
            "mbed-os/features/netsocket/NetworkInterface.h",
            "mbed-os/features/netsocket/NetworkStack.cpp",
            "mbed-os/features/netsocket/NetworkStack.h",
            "mbed-os/features/netsocket/Socket.cpp",
            "mbed-os/features/netsocket/Socket.h",
            "mbed-os/features/netsocket/SocketAddress.cpp",
            "mbed-os/features/netsocket/SocketAddress.h",
            "mbed-os/features/netsocket/TCPServer.cpp",
            "mbed-os/features/netsocket/TCPServer.h",
            "mbed-os/features/netsocket/TCPSocket.cpp",
            "mbed-os/features/netsocket/TCPSocket.h",
            "mbed-os/features/netsocket/UDPSocket.cpp",
            "mbed-os/features/netsocket/UDPSocket.h",
            "mbed-os/features/netsocket/WiFiAccessPoint.cpp",
            "mbed-os/features/netsocket/WiFiAccessPoint.h",
            "mbed-os/features/netsocket/WiFiInterface.h",
            "mbed-os/features/netsocket/cellular/generic_modem_driver/OnboardCellularInterface.cpp",
            "mbed-os/features/netsocket/cellular/generic_modem_driver/OnboardCellularInterface.h",
            "mbed-os/features/netsocket/cellular/generic_modem_driver/PPPCellularInterface.cpp",
            "mbed-os/features/netsocket/cellular/generic_modem_driver/PPPCellularInterface.h",
            "mbed-os/features/netsocket/cellular/generic_modem_driver/UARTCellularInterface.cpp",
            "mbed-os/features/netsocket/cellular/generic_modem_driver/UARTCellularInterface.h",
            "mbed-os/features/netsocket/cellular/onboard_modem_api.h",
            "mbed-os/features/netsocket/cellular/utils/APN_db.h",
            "mbed-os/features/netsocket/emac_stack_mem.h",
            "mbed-os/features/netsocket/nsapi.h",
            "mbed-os/features/netsocket/nsapi_dns.cpp",
            "mbed-os/features/netsocket/nsapi_dns.h",
            "mbed-os/features/netsocket/nsapi_ppp.h",
            "mbed-os/features/netsocket/nsapi_types.h",
            "mbed-os/hal/analogin_api.h",
            "mbed-os/hal/analogout_api.h",
            "mbed-os/hal/buffer.h",
            "mbed-os/hal/can_api.h",
            "mbed-os/hal/can_helper.h",
            "mbed-os/hal/dma_api.h",
            "mbed-os/hal/emac_api.h",
            "mbed-os/hal/ethernet_api.h",
            "mbed-os/hal/flash_api.h",
            "mbed-os/hal/gpio_api.h",
            "mbed-os/hal/gpio_irq_api.h",
            "mbed-os/hal/i2c_api.h",
            "mbed-os/hal/lp_ticker_api.h",
            "mbed-os/hal/mbed_flash_api.c",
            "mbed-os/hal/mbed_gpio.c",
            "mbed-os/hal/mbed_lp_ticker_api.c",
            "mbed-os/hal/mbed_pinmap_common.c",
            "mbed-os/hal/mbed_sleep_manager.c",
            "mbed-os/hal/mbed_ticker_api.c",
            "mbed-os/hal/mbed_us_ticker_api.c",
            "mbed-os/hal/pinmap.h",
            "mbed-os/hal/port_api.h",
            "mbed-os/hal/pwmout_api.h",
            "mbed-os/hal/rtc_api.h",
            "mbed-os/hal/serial_api.h",
            "mbed-os/hal/sleep_api.h",
            "mbed-os/hal/spi_api.h",
            "mbed-os/hal/storage_abstraction/Driver_Common.h",
            "mbed-os/hal/storage_abstraction/Driver_Storage.h",
            "mbed-os/hal/ticker_api.h",
            "mbed-os/hal/trng_api.h",
            "mbed-os/hal/us_ticker_api.h",
            "mbed-os/mbed.h",
            "mbed-os/platform/ATCmdParser.cpp",
            "mbed-os/platform/ATCmdParser.h",
            "mbed-os/platform/CThunk.h",
            "mbed-os/platform/CallChain.cpp",
            "mbed-os/platform/CallChain.h",
            "mbed-os/platform/Callback.h",
            "mbed-os/platform/CircularBuffer.h",
            "mbed-os/platform/CriticalSectionLock.h",
            "mbed-os/platform/DeepSleepLock.h",
            "mbed-os/platform/DirHandle.h",
            "mbed-os/platform/FileBase.cpp",
            "mbed-os/platform/FileBase.h",
            "mbed-os/platform/FileHandle.cpp",
            "mbed-os/platform/FileHandle.h",
            "mbed-os/platform/FileLike.h",
            "mbed-os/platform/FilePath.cpp",
            "mbed-os/platform/FilePath.h",
            "mbed-os/platform/FileSystemHandle.cpp",
            "mbed-os/platform/FileSystemHandle.h",
            "mbed-os/platform/FileSystemLike.h",
            "mbed-os/platform/FunctionPointer.h",
            "mbed-os/platform/LocalFileSystem.cpp",
            "mbed-os/platform/LocalFileSystem.h",
            "mbed-os/platform/NonCopyable.h",
            "mbed-os/platform/PlatformMutex.h",
            "mbed-os/platform/SingletonPtr.h",
            "mbed-os/platform/Stream.cpp",
            "mbed-os/platform/Stream.h",
            "mbed-os/platform/Transaction.h",
            "mbed-os/platform/critical.h",
            "mbed-os/platform/mbed_alloc_wrappers.cpp",
            "mbed-os/platform/mbed_application.c",
            "mbed-os/platform/mbed_application.h",
            "mbed-os/platform/mbed_assert.c",
            "mbed-os/platform/mbed_assert.h",
            "mbed-os/platform/mbed_board.c",
            "mbed-os/platform/mbed_critical.c",
            "mbed-os/platform/mbed_critical.h",
            "mbed-os/platform/mbed_debug.h",
            "mbed-os/platform/mbed_error.c",
            "mbed-os/platform/mbed_error.h",
            "mbed-os/platform/mbed_interface.c",
            "mbed-os/platform/mbed_interface.h",
            "mbed-os/platform/mbed_mem_trace.c",
            "mbed-os/platform/mbed_mem_trace.h",
            "mbed-os/platform/mbed_mktime.c",
            "mbed-os/platform/mbed_mktime.h",
            "mbed-os/platform/mbed_poll.cpp",
            "mbed-os/platform/mbed_poll.h",
            "mbed-os/platform/mbed_preprocessor.h",
            "mbed-os/platform/mbed_retarget.cpp",
            "mbed-os/platform/mbed_retarget.h",
            "mbed-os/platform/mbed_rtc_time.cpp",
            "mbed-os/platform/mbed_rtc_time.h",
            "mbed-os/platform/mbed_sdk_boot.c",
            "mbed-os/platform/mbed_semihost_api.c",
            "mbed-os/platform/mbed_semihost_api.h",
            "mbed-os/platform/mbed_sleep.h",
            "mbed-os/platform/mbed_stats.c",
            "mbed-os/platform/mbed_stats.h",
            "mbed-os/platform/mbed_toolchain.h",
            "mbed-os/platform/mbed_wait_api.h",
            "mbed-os/platform/mbed_wait_api_no_rtos.c",
            "mbed-os/platform/mbed_wait_api_rtos.cpp",
            "mbed-os/platform/platform.h",
            "mbed-os/platform/rtc_time.h",
            "mbed-os/platform/semihost_api.h",
            "mbed-os/platform/sleep.h",
            "mbed-os/platform/toolchain.h",
            "mbed-os/platform/wait_api.h",
            "mbed-os/rtos/ConditionVariable.cpp",
            "mbed-os/rtos/ConditionVariable.h",
            "mbed-os/rtos/EventFlags.cpp",
            "mbed-os/rtos/EventFlags.h",
            "mbed-os/rtos/Mail.h",
            "mbed-os/rtos/MemoryPool.h",
            "mbed-os/rtos/Mutex.cpp",
            "mbed-os/rtos/Mutex.h",
            "mbed-os/rtos/Queue.h",
            "mbed-os/rtos/RtosTimer.cpp",
            "mbed-os/rtos/RtosTimer.h",
            "mbed-os/rtos/Semaphore.cpp",
            "mbed-os/rtos/Semaphore.h",
            "mbed-os/rtos/TARGET_CORTEX/mbed_boot.c",
            "mbed-os/rtos/TARGET_CORTEX/mbed_rtos1_types.h",
            "mbed-os/rtos/TARGET_CORTEX/mbed_rtos_storage.h",
            "mbed-os/rtos/TARGET_CORTEX/mbed_rtx_conf.h",
            "mbed-os/rtos/TARGET_CORTEX/mbed_rtx_handlers.c",
            "mbed-os/rtos/TARGET_CORTEX/mbed_rtx_idle.cpp",
            "mbed-os/rtos/TARGET_CORTEX/rtx4/cmsis_os.h",
            "mbed-os/rtos/TARGET_CORTEX/rtx4/cmsis_os1.c",
            "mbed-os/rtos/TARGET_CORTEX/rtx5/Include/cmsis_os2.h",
            "mbed-os/rtos/TARGET_CORTEX/rtx5/Include/os_tick.h",
            "mbed-os/rtos/TARGET_CORTEX/rtx5/RTX/Config/RTX_Config.c",
            "mbed-os/rtos/TARGET_CORTEX/rtx5/RTX/Config/RTX_Config.h",
            "mbed-os/rtos/TARGET_CORTEX/rtx5/RTX/Include/rtx_evr.h",
            "mbed-os/rtos/TARGET_CORTEX/rtx5/RTX/Include/rtx_os.h",
            "mbed-os/rtos/TARGET_CORTEX/rtx5/RTX/Source/TOOLCHAIN_GCC/TARGET_RTOS_M4_M7/irq_cm4f.S",
            "mbed-os/rtos/TARGET_CORTEX/rtx5/RTX/Source/rt_OsEventObserver.c",
            "mbed-os/rtos/TARGET_CORTEX/rtx5/RTX/Source/rt_OsEventObserver.h",
            "mbed-os/rtos/TARGET_CORTEX/rtx5/RTX/Source/rtx_core_c.h",
            "mbed-os/rtos/TARGET_CORTEX/rtx5/RTX/Source/rtx_core_ca.h",
            "mbed-os/rtos/TARGET_CORTEX/rtx5/RTX/Source/rtx_core_cm.h",
            "mbed-os/rtos/TARGET_CORTEX/rtx5/RTX/Source/rtx_delay.c",
            "mbed-os/rtos/TARGET_CORTEX/rtx5/RTX/Source/rtx_evflags.c",
            "mbed-os/rtos/TARGET_CORTEX/rtx5/RTX/Source/rtx_evr.c",
            "mbed-os/rtos/TARGET_CORTEX/rtx5/RTX/Source/rtx_kernel.c",
            "mbed-os/rtos/TARGET_CORTEX/rtx5/RTX/Source/rtx_lib.c",
            "mbed-os/rtos/TARGET_CORTEX/rtx5/RTX/Source/rtx_lib.h",
            "mbed-os/rtos/TARGET_CORTEX/rtx5/RTX/Source/rtx_memory.c",
            "mbed-os/rtos/TARGET_CORTEX/rtx5/RTX/Source/rtx_mempool.c",
            "mbed-os/rtos/TARGET_CORTEX/rtx5/RTX/Source/rtx_msgqueue.c",
            "mbed-os/rtos/TARGET_CORTEX/rtx5/RTX/Source/rtx_mutex.c",
            "mbed-os/rtos/TARGET_CORTEX/rtx5/RTX/Source/rtx_semaphore.c",
            "mbed-os/rtos/TARGET_CORTEX/rtx5/RTX/Source/rtx_system.c",
            "mbed-os/rtos/TARGET_CORTEX/rtx5/RTX/Source/rtx_thread.c",
            "mbed-os/rtos/TARGET_CORTEX/rtx5/RTX/Source/rtx_timer.c",
            "mbed-os/rtos/TARGET_CORTEX/rtx5/Source/os_systick.c",
            "mbed-os/rtos/TARGET_CORTEX/rtx5/Source/os_tick_ptim.c",
            "mbed-os/rtos/Thread.cpp",
            "mbed-os/rtos/Thread.h",
            "mbed-os/rtos/rtos.h",
            "mbed-os/rtos/rtos_idle.h",
            "mbed-os/targets/TARGET_STM/PeripheralPins.h",
            "mbed-os/targets/TARGET_STM/PinNamesTypes.h",
            "mbed-os/targets/TARGET_STM/PortNames.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/TARGET_STM32L476xG/TARGET_NUCLEO_L476RG/PeripheralNames.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/TARGET_STM32L476xG/TARGET_NUCLEO_L476RG/PeripheralPins.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/TARGET_STM32L476xG/TARGET_NUCLEO_L476RG/PinNames.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/TARGET_STM32L476xG/TARGET_NUCLEO_L476RG/system_clock.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/TARGET_STM32L476xG/device/TOOLCHAIN_GCC_ARM/startup_stm32l476xx.S",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/TARGET_STM32L476xG/device/cmsis.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/TARGET_STM32L476xG/device/cmsis_nvic.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/TARGET_STM32L476xG/device/hal_tick.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/TARGET_STM32L476xG/device/stm32l476xx.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/TARGET_STM32L476xG/device/stm32l4xx.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/TARGET_STM32L476xG/device/system_stm32l4xx.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/TARGET_STM32L476xG/objects.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/analogin_device.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/analogout_device.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/can_device.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/common_objects.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32_hal_legacy.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_adc.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_adc.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_adc_ex.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_adc_ex.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_can.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_can.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_comp.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_comp.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_conf.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_cortex.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_cortex.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_crc.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_crc.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_crc_ex.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_crc_ex.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_cryp.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_cryp.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_cryp_ex.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_cryp_ex.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_dac.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_dac.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_dac_ex.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_dac_ex.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_dcmi.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_dcmi.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_def.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_dfsdm.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_dfsdm.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_dma.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_dma.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_dma2d.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_dma2d.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_firewall.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_firewall.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_flash.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_flash.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_flash_ex.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_flash_ex.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_flash_ramfunc.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_flash_ramfunc.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_gpio.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_gpio.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_gpio_ex.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_hash.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_hash.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_hash_ex.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_hash_ex.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_hcd.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_hcd.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_i2c.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_i2c.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_i2c_ex.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_i2c_ex.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_irda.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_irda.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_irda_ex.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_iwdg.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_iwdg.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_lcd.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_lcd.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_lptim.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_lptim.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_nand.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_nand.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_nor.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_nor.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_opamp.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_opamp.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_opamp_ex.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_opamp_ex.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_pcd.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_pcd.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_pcd_ex.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_pcd_ex.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_pwr.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_pwr.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_pwr_ex.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_pwr_ex.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_qspi.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_qspi.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_rcc.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_rcc.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_rcc_ex.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_rcc_ex.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_rng.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_rng.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_rtc.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_rtc.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_rtc_ex.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_rtc_ex.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_sai.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_sai.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_sd.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_sd.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_smartcard.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_smartcard.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_smartcard_ex.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_smartcard_ex.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_smbus.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_smbus.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_spi.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_spi.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_spi_ex.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_spi_ex.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_sram.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_sram.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_swpmi.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_swpmi.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_tim.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_tim.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_tim_ex.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_tim_ex.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_tsc.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_tsc.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_uart.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_uart.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_uart_ex.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_uart_ex.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_usart.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_usart.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_usart_ex.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_wwdg.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_hal_wwdg.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_adc.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_adc.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_bus.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_comp.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_comp.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_cortex.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_crc.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_crc.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_crs.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_crs.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_dac.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_dac.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_dma.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_dma.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_dma2d.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_dma2d.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_exti.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_exti.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_fmc.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_fmc.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_gpio.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_gpio.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_i2c.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_i2c.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_iwdg.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_lptim.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_lptim.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_lpuart.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_lpuart.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_opamp.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_opamp.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_pwr.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_pwr.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_rcc.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_rcc.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_rng.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_rng.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_rtc.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_rtc.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_sdmmc.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_sdmmc.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_spi.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_spi.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_swpmi.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_swpmi.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_system.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_tim.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_tim.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_usart.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_usart.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_usb.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_usb.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_utils.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_utils.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/stm32l4xx_ll_wwdg.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/system_stm32l4xx.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/flash_api.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/gpio_irq_device.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/gpio_irq_device.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/i2c_device.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/pin_device.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/pwmout_device.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/pwmout_device.h",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/serial_device.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/spi_api.c",
            "mbed-os/targets/TARGET_STM/TARGET_STM32L4/spi_device.h",
            "mbed-os/targets/TARGET_STM/analogin_api.c",
            "mbed-os/targets/TARGET_STM/analogout_api.c",
            "mbed-os/targets/TARGET_STM/can_api.c",
            "mbed-os/targets/TARGET_STM/gpio_api.c",
            "mbed-os/targets/TARGET_STM/gpio_irq_api.c",
            "mbed-os/targets/TARGET_STM/gpio_object.h",
            "mbed-os/targets/TARGET_STM/hal_tick_16b.c",
            "mbed-os/targets/TARGET_STM/hal_tick_32b.c",
            "mbed-os/targets/TARGET_STM/i2c_api.c",
            "mbed-os/targets/TARGET_STM/lp_ticker.c",
            "mbed-os/targets/TARGET_STM/mbed_overrides.c",
            "mbed-os/targets/TARGET_STM/mbed_rtx.h",
            "mbed-os/targets/TARGET_STM/nvic_addr.h",
            "mbed-os/targets/TARGET_STM/pinmap.c",
            "mbed-os/targets/TARGET_STM/port_api.c",
            "mbed-os/targets/TARGET_STM/pwmout_api.c",
            "mbed-os/targets/TARGET_STM/rtc_api.c",
            "mbed-os/targets/TARGET_STM/rtc_api_hal.h",
            "mbed-os/targets/TARGET_STM/serial_api.c",
            "mbed-os/targets/TARGET_STM/serial_api_hal.h",
            "mbed-os/targets/TARGET_STM/sleep.c",
            "mbed-os/targets/TARGET_STM/stm32_assert.h",
            "mbed-os/targets/TARGET_STM/stm_spi_api.c",
            "mbed-os/targets/TARGET_STM/trng_api.c",
            "mbed-os/targets/TARGET_STM/us_ticker_16b.c",
            "mbed-os/targets/TARGET_STM/us_ticker_32b.c",
        ]
    }

    cpp.defines: [
        "ARM_MATH_CM4",
        "DEVICE_ANALOGIN=1",
        "DEVICE_ANALOGOUT=1",
        "DEVICE_CAN=1",
        "DEVICE_FLASH=1",
        "DEVICE_I2C=1",
        "DEVICE_I2CSLAVE=1",
        "DEVICE_I2C_ASYNCH=1",
        "DEVICE_INTERRUPTIN=1",
        "DEVICE_LOWPOWERTIMER=1",
        "DEVICE_PORTIN=1",
        "DEVICE_PORTINOUT=1",
        "DEVICE_PORTOUT=1",
        "DEVICE_PWMOUT=1",
        "DEVICE_RTC=1",
        "DEVICE_SERIAL=1",
        "DEVICE_SERIAL_ASYNCH=1",
        "DEVICE_SERIAL_FC=1",
        "DEVICE_SLEEP=1",
        "DEVICE_SPI=1",
        "DEVICE_SPISLAVE=1",
        "DEVICE_SPI_ASYNCH=1",
        "DEVICE_STDIO_MESSAGES=1",
        "DEVICE_TRNG=1",
        "MBED_BUILD_TIMESTAMP=1577311020.89",
        "TARGET_CORTEX",
        "TARGET_CORTEX_M",
        "TARGET_FAMILY_STM32",
        "TARGET_FF_ARDUINO",
        "TARGET_FF_MORPHO",
        "TARGET_LIKE_CORTEX_M4",
        "TARGET_LIKE_MBED",
        "TARGET_M4",
        "TARGET_NAME=NUCLEO_L476RG",
        "TARGET_NUCLEO_L476RG",
        "TARGET_RELEASE",
        "TARGET_RTOS_M4_M7",
        "TARGET_STM",
        "TARGET_STM32L4",
        "TARGET_STM32L476RG",
        "TARGET_STM32L476xG",
        "TOOLCHAIN_GCC",
        "TOOLCHAIN_GCC_ARM",
        "TRANSACTION_QUEUE_SIZE_SPI=2",
        "USBHOST_OTHER",
        "__CMSIS_RTOS",
        "__CORTEX_M4",
        "__FPU_PRESENT=1",
        "__MBED_CMSIS_RTOS_CM",
        "__MBED__=1",

        // Configuration parameters
        "CLOCK_SOURCE=USE_PLL_HSE_XTAL",//"USE_PLL_MSI",        // set by target:NUCLEO_L476RG
        "MBED_CONF_DRIVERS_UART_SERIAL_RXBUF_SIZE=256",         // set by library:drivers
        "MBED_CONF_DRIVERS_UART_SERIAL_TXBUF_SIZE=256",         // set by library:drivers
        "MBED_CONF_EVENTS_PRESENT=1",                           // set by library:events
        "MBED_CONF_EVENTS_SHARED_DISPATCH_FROM_APPLICATION=0",  // set by library:events
        "MBED_CONF_EVENTS_SHARED_EVENTSIZE=256",                // set by library:events
        "MBED_CONF_EVENTS_SHARED_HIGHPRIO_EVENTSIZE=256",       // set by library:events
        "MBED_CONF_EVENTS_SHARED_HIGHPRIO_STACKSIZE=1024",      // set by library:events
        "MBED_CONF_EVENTS_SHARED_STACKSIZE=1024",               // set by library:events
        "MBED_CONF_EVENTS_USE_LOWPOWER_TIMER_TICKER=0",         // set by library:events
        "MBED_CONF_FILESYSTEM_PRESENT=1",                       // set by library:filesystem
        "MBED_CONF_NSAPI_PRESENT=1",                            // set by library:nsapi
        "MBED_CONF_PLATFORM_DEFAULT_SERIAL_BAUD_RATE=115200",   // set by library:platform
        "MBED_CONF_PLATFORM_FORCE_NON_COPYABLE_ERROR=0",        // set by library:platform
        "MBED_CONF_PLATFORM_STDIO_BAUD_RATE=115200",            // set by library:platform
        "MBED_CONF_PLATFORM_STDIO_CONVERT_NEWLINES=0",          // set by library:platform
        "MBED_CONF_PLATFORM_STDIO_FLUSH_AT_EXIT=1",             // set by library:platform
        "MBED_CONF_PPP_CELL_IFACE_APN_LOOKUP=0",                // set by library:ppp-cell-iface
        "MBED_CONF_PPP_CELL_IFACE_AT_PARSER_BUFFER_SIZE=256",   // set by library:ppp-cell-iface
        "MBED_CONF_PPP_CELL_IFACE_AT_PARSER_TIMEOUT=8000",      // set by library:ppp-cell-iface
        "MBED_CONF_PPP_CELL_IFACE_BAUD_RATE=115200",            // set by library:ppp-cell-iface
        "MBED_CONF_RTOS_PRESENT=1",                             // set by library:rtos
    ]

    cpp.cxxFlags: [
        "-std=gnu++14",
        "-fno-rtti",
    ]

    cpp.cFlags: [
        "-std=gnu11",
    ]

    cpp.commonCompilerFlags: [
        //"-include",F
        //"mbed_config.h",
        "-MMD",
        "-Wall",
        "-Wextra",
        "-Wno-missing-field-initializers",
        "-Wno-unused-parameter",
        "-Wvla",
        "-c",
        "-fdata-sections",
        "-ffunction-sections",
        "-fmessage-length=0",
        "-fno-delete-null-pointer-checks",
        "-fno-exceptions",
        "-fomit-frame-pointer",
        "-funsigned-char",
        "-g",
        mcpu,
        mfloatAbi,
        mfpu,
        "-mthumb",
    ]
    cpp.driverLinkerFlags:[
        //        "-E",
        //        "-P",
        //        "-Wl,--gc-sections",
        //        "-Wl,--wrap,main",
        "-Wl,--wrap,_malloc_r",
        "-Wl,--wrap,_free_r",
        //        "-Wl,--wrap,_realloc_r",
        //        "-Wl,--wrap,_memalign_r",
        //        "-Wl,--wrap,_calloc_r",
        "-Wl,--wrap,exit",
        //        "-Wl,--wrap,atexit",
        "-Wl,-n",
        mcpu,
        "-mthumb",
    ]

    cpp.driverFlags: [
        mcpu,
        mfloatAbi,
        mfpu,
        "-mthumb",
        "-Xlinker",
        "--gc-sections",
        "-specs=nosys.specs",
        "-specs=nano.specs",
        "-u_printf_float",
    ]

    //    cpp.libraryPaths: [
    //        Home + "/Middlewares/ST/STemWin/Lib",
    //        STemWin + "/STemWin_Addons",
    //    ]
    //    cpp.staticLibraries: [
    //        ":STemWin540_CM7_OS_GCC_ot.a",
    //        ":STM32746G_Discovery_STemWin_Addons_GCC.a",
    //    ]
    cpp.linkerFlags:[
        "-flto",
        "--gc-sections",
        "-lc",
        "-lgcc",
        "-lm",
        "-lnosys",
        "-lstdc++",
        "-lsupc++",
        //"--start-group",
        "-T" + path + "/mbed-os/targets/TARGET_STM/TARGET_STM32L4/TARGET_STM32L476xG/device/TOOLCHAIN_GCC_ARM/STM32L476XX.ld",
    ]



    cpp.includePaths: [
        "src/Encoder/",
        "src/TextLCD/",
        //        "src/DS18B20/",
        //        "src/OneWire_1/",
        "src/OneWire/",
        "src/OneWire/devices/",

        "mbed-os/",
        "mbed-os/cmsis/TARGET_CORTEX_M/",
        "mbed-os/drivers/",
        "mbed-os/events/",
        "mbed-os/events/equeue/",
        "mbed-os/features/",
        "mbed-os/features/filesystem/",
        "mbed-os/features/filesystem/bd/",
        "mbed-os/features/filesystem/fat/",
        "mbed-os/features/filesystem/fat/ChaN/",
        "mbed-os/features/frameworks/",
        "mbed-os/features/frameworks/greentea-client/",
        "mbed-os/features/frameworks/greentea-client/greentea-client/",
        "mbed-os/features/frameworks/unity/",
        "mbed-os/features/frameworks/unity/unity/",
        "mbed-os/features/frameworks/utest/",
        "mbed-os/features/frameworks/utest/utest/",
        "mbed-os/features/mbedtls/",
        "mbed-os/features/mbedtls/inc/",
        "mbed-os/features/mbedtls/inc/mbedtls/",
        "mbed-os/features/mbedtls/platform/",
        "mbed-os/features/mbedtls/platform/inc/",
        "mbed-os/features/mbedtls/targets/TARGET_STM/",
        "mbed-os/features/netsocket/",
        "mbed-os/features/netsocket/cellular/",
        "mbed-os/features/netsocket/cellular/generic_modem_driver/",
        "mbed-os/features/netsocket/cellular/utils/",
        "mbed-os/hal/",
        "mbed-os/hal/storage_abstraction/",
        "mbed-os/platform/",
        "mbed-os/rtos/",
        "mbed-os/rtos/TARGET_CORTEX/",
        "mbed-os/rtos/TARGET_CORTEX/rtx4/",
        "mbed-os/rtos/TARGET_CORTEX/rtx5/",
        "mbed-os/rtos/TARGET_CORTEX/rtx5/Include/",
        "mbed-os/rtos/TARGET_CORTEX/rtx5/RTX/",
        "mbed-os/rtos/TARGET_CORTEX/rtx5/RTX/Config/",
        "mbed-os/rtos/TARGET_CORTEX/rtx5/RTX/Include/",
        "mbed-os/rtos/TARGET_CORTEX/rtx5/RTX/Source/",
        "mbed-os/targets/TARGET_STM/",
        "mbed-os/targets/TARGET_STM/TARGET_STM32L4/",
        "mbed-os/targets/TARGET_STM/TARGET_STM32L4/TARGET_STM32L476xG/",
        "mbed-os/targets/TARGET_STM/TARGET_STM32L4/TARGET_STM32L476xG/TARGET_NUCLEO_L476RG/",
        "mbed-os/targets/TARGET_STM/TARGET_STM32L4/TARGET_STM32L476xG/device/",
        "mbed-os/targets/TARGET_STM/TARGET_STM32L4/device/",
        path,
    ]

    Properties {
        condition: qbs.buildVariant === "debug"
        cpp.defines: outer.concat(["DEBUG=1"])
        cpp.debugInformation: true
        cpp.optimization: "none"
    }

    Properties {
        condition: qbs.buildVariant === "release"
        cpp.defines: outer.concat(["DEBUG=0"])
        cpp.debugInformation: false
        cpp.optimization: "small"
        // Виды оптимизаций
        // Types of optimizations
        // "none", "fast", "small"
    }

    Group {
        qbs.install: true
        fileTagsFilter: "application"
    }


    //    Group {
    //        name: "app"
    //        prefix: path//"/"
    //        files: [
    //            //            "Src/*.c",
    //            //            ""Src/*.cpp",",
    //            //            ""Inc/*.h",",
    //            "/**/*.c",
    //            ""/**/*.cpp",",
    //            ""/**/*.h",",
    //            ""*.cpp",",
    //            ""*.h",",
    //        ]
    //        excludeFiles: [
    //        ]
    //        //        cpp.cxxFlags: [ "-std=c++11" ]
    //        //        cpp.cFlags: [ "-std=gnu99" ]
    //        //        cpp.warningLevel: "all"
    //    }


    Rule
    {
        inputs: ["application"]

        Artifact
        {
            filePath: project.buildDirectory + product.name + ".hex"
            fileTags: "flash"
        }
        //        Artifact
        //        {
        //            filePath: project.buildDirectory + product.name + "".h",ex"
        //            fileTags: "flash"
        //        }
        prepare:
        {
            var GCCPath = ""//"C:/STM32/CubeIDE_1.1.0/STM32CubeIDE/plugins/com.st.stm32cube.ide.mcu.externaltools.gnu-tools-for-stm32.7-2018-q2-update.win32_1.0.0.201904181610/tools/bin"
            var OpenOCDPath = "C:/STM32/CubeIDE_1.1.0/STM32CubeIDE/plugins/com.st.stm32cube.ide.mcu.externaltools.openocd.win32_1.1.0.201910081157/tools"
            var OpenOCDInterface = "stlink-v2.cfg"
            var OpenOCDTarget = "stm32f1x.cfg" // "stm32f0x.cfg"

            var argsSize = [input.filePath]
            var argsObjcopy = ["-O", "ihex", input.filePath, output.filePath]
            var argsObjcopy2 = ["-O", "binary",
                                "-S",
                                "-g",
                                //"-N",
                                input.filePath, project.buildDirectory + product.name + ".bin"]

            var argsFlashing =
                    [
                        "-f", OpenOCDPath + "/scripts/interface/" + OpenOCDInterface,
                        "-f", OpenOCDPath + "/scripts/target/" + OpenOCDTarget,
                        "-c", "init",
                        "-c", "halt",
                        "-c", "flash write_image erase " + input.filePath,
                        "-c", "reset",
                        "-c", "shutdown"
                    ]

            var cmdSize = new Command(GCCPath + "arm-none-eabi-size.exe", argsSize)
            var cmdObjcopy = new Command(GCCPath + "arm-none-eabi-objcopy.exe", argsObjcopy)
            var cmdObjcopy2 = new Command(GCCPath + "arm-none-eabi-objcopy.exe", argsObjcopy2)
            //                var cmdFlash = new Command(OpenOCDPath + "/bin/openocd.exe", argsFlashing);

            cmdSize.description = "Size of sections:"
            "cmdSize.h",ighlight = "linker"

            cmdObjcopy.description = "convert to bin..."
            "cmdObjcopy.h",ighlight = "linker"

            cmdObjcopy2.description = "convert to bin..."
            "cmdObjcopy2.h",ighlight = "linker"

            //                cmdFlash.description = "download firmware to uC..."
            //                "cmdFlash.h",ighlight = "linker"

            return [
                        cmdSize,
                        cmdObjcopy,
                        cmdObjcopy2,
                        //                            cmdFlash
                    ]
        }
    }
}
//}
